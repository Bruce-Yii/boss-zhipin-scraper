...
### 调整
- **批次冷却默认时长 240s → 60s**（`--cool-seconds` 默认值；用户拍板）。

## v2.18.2 (2026-09-24)

### 优化（DOM 通道再优化 / #78）
- **资源拦截扩展**：详情页 XHR 审计（25 个）——补齐埋点域 `shink.zhipin.com` / `wapi/zpApm/*`；新增 UI 装饰拦截（弹窗/横幅/引导/竞品推荐等 9 条模式）。关键接口（`zppassport`/`getStatus`/`city.json` 等）明确不在名单。
- **批次冷却参数**：`--cool-every N --cool-seconds M`（默认 0=关闭；大批量建议 250/240）——**主动冷却比被风控停更安全**（被停会写环境标记）；串行/并行/面板三处循环均支持。
- **冷却可选重启 Chrome**：`--cool-restart-chrome`（实测连跑千条后 2s/条退化到 8s/条，重启即恢复）；串行模式重启后自动清空并重建共享会话。
- **API tab 坏档重建**：security 模式共享 tab 失败/失效后同样重建（此前仅 encrypt 模式有重建逻辑）。
...
### 优化（资源拦截扩展追踪域 / #67）
- `DOM_BLOCK_URL_PATTERNS` 追加 `*hm.baidu.com*` / `*apm-fe.zhipin.com*` / `*logapi.zhipin.com*`：详情页资源审计（performance API 实测 116 请求/页）显示追踪类约 11 个/页、拦掉零内容风险；图片类会话热后全走缓存（0 字节），继续放行保人形瀑布；未知第一方端点（t/shink）不动。
...
### 新增（#31 排期准备 · 实验旋钮 / #61）
- **`BOSS_BURST_*` 环境变量覆盖 `BurstThrottle`**（center/min_delay/max_delay + short/long 阈值）：#31 档位实验（0.20/0.30/0.44）免改码入口——如 0.33 档 `BOSS_BURST_CENTER/MIN/MAX=3.0`，采集端 burst 阈值 `SHORT=12/LONG=32`。非法/越界/min>max 时回退默认；**无环境变量时默认值零变化**（节律红线：默认调整仍需实测边界 + 用户拍板）。

## v2.16.5 (2026-09-23)

### 修复（面板读旧内容 / #57，真机实测 8/8 同一 JD）
- **右面板等待目标岗位切换**：点击前记录面板文本指纹；点击后 `_wait_for_panel_job` 轮询 `PANEL_JOB_STATE_JS`（面板存在 + JD 区 ≥80 字 + **标题与 company 同时命中**，或面板指纹相对快照变化——格式漂移兜底）；超时/异常 → `invalid_detail"右面板未切换到目标岗位"` 保守失败。旧逻辑"面板存在即读"在初始首卡面板下立即通过，直接把旧内容当新 JD。
- **卡片定位增强**：长标题列表卡常截断，`CLICK_CARD_JS` 增加标题前 12 字前缀匹配（此前 2/10"搜索页未找到该岗位卡片"）。
- 注：面板 JD 完整性（截断？`page_update_date`？）仍需真机对照；本次只修"读错岗"致命项。
...
- **`--dom-gap MIN,MAX`（默认不变 4,9）**：DOM 族详情岗位间隔实验旋钮（串行 DOM 与右面板共用）；解析约束 `0 < MIN <= MAX <= 120`，非法值 exit 2。**默认值是节律红线**——调整须等 #31 实测边界 + 用户拍板；本参数仅为档位实验（DOM 3–6s / 面板 2–4s）提供免改码入口。
