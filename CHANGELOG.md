...
### 变更
- **BurstThrottle 节律回到安全区中心（~0.44 req/s）**：旧阈值 `short=3 / long=6` 在稳态 ~0.44 req/s 下也会被反复触发（45s 内约 20 次 ≥6），把有效速率从设计意图 ~0.44 拖到**实测 ~0.13 req/s**（2026-09-22 离线实测：14 次平均 5.46s/次、稳态 7–9s/次）。新阈值 `short=10 / long=24` 让**稳态不再触发惩罚**（惩罚仅作真突发兜底保留）；`min_delay=1.5s`（0.67 req/s 安全上界）不变。
- **受控实抓验证**（杭州 × 1 页 × 8 详情，新默认）：**详情 8/8 成功、0 失败，27s、平均 ~3.4s/条，无 `code 37`**；整格端到端 **54.8s**。
