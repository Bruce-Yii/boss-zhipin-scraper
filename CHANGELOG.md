...
### 变更
- **BurstThrottle 节律回到安全区中心（~0.44 req/s）**：旧阈值 `short=3 / long=6` 在稳态 ~0.44 req/s 下也会被反复触发（45s 内约 20 次 ≥6），把有效速率从设计意图 ~0.44 拖到**实测 ~0.13 req/s**（2026-09-22 离线实测：14 次平均 5.46s/次、稳态 7–9s/次）。据同行专题（30 仓）交叉核对——**同行 boss-agent-cli / BossHunter 的 burst 惩罚仅用于"发送端"，采集端不加 burst**——故采集端把阈值设到 `min_delay=1.5s` 理论最大频次之上：`short=12 / long=32`（正常运行永不触发=采集端不加 burst，阈值仅作死代码兜底）；`min_delay=1.5s`（0.67 req/s 安全上界）不变。
- **受控实抓验证**（杭州 × 1 页 × 8 详情）：**详情 8/8 成功、0 失败，27s、平均 ~3.4s/条，无 `code 37`**；整格端到端 **54.8s**。（注：30 详情档曾触 37，但当时账号被另一进程打热，无法干净归因——见 `节律边界实测方案`。）
- 测试 +1（`RateLimitCenterTests` 锁定默认参数）
