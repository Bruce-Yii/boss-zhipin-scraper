# BOSS直聘爬虫 · 职位抓取工具 v2.17（Chrome CDP / 明文薪资）
...
| `--list-mode xhr / passive` | 列表通道：`xhr`=注入 XHR 拉 wapi（默认，快，配合 `--pages-parallel`）；`passive`=Network 域被动捕获页面自身 joblist 响应（零注入请求，消除注入 XHR 触发的 code 37，上游 #55；与 `--pages-parallel` 互斥自动回串行） |
| `--analysis` | 分析报告 |
