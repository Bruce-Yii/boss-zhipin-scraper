...
- 可选字段（列表抓取时）：`exhausted`（bool，本 run 是否翻到底：任一副页返回 <30 条 → true；`--pages 1` 单页 <30 条 → true；满页未到底 → false。规格侧抽样感知下架判定依据；随契约 v2 正式纳入，字段缺失按 false 处理）、`job_valid_status`（BOSS 官方在招状态，可校准下架推断）、`icon_flags`/`icon_word`（"急"/"新"等平台标签）、`proxy_job`/`proxy_type`（代招标记：猎头/外包）、`job_type`（岗位类型编码）
