...
- **详情抓取主路径 = API 通道**（2026-08-14 起）：`/wapi/zpgeek/job/detail.json?jobId=&securityId=&city=` 每岗 1 次轻量 XHR（JD 秒回）；`securityId` 由列表阶段内存收集（`scrape_list` 返回 `security_map`，**不落导出文件**——红线）；`--input` 老文件无 securityId 自动回退 DOM 渲染（`_scrape_one_detail_via_api` vs `_scrape_one_detail` 分支）。详情 API 风控码（code!=0）→ risk_timeout 全停。
