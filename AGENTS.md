...
- **显式双通道 + securityId 受限 sidecar**（2026-09-22 P1）：详情按有无 `securityId` 走 API/DOM；导出 meta 记 `detail_channel`（`api`/`dom`/`mixed`），`--input` 未命中凭据时**明确告警**。`securityId`（每岗短期接口令牌，**非登录凭据**）允许落到 `~/.boss-zhipin-scraper/.session/`（**仓库外/`chmod 600`/60min TTL/run 结束即删/启动清理**），供 `--input` 续抓复用；**绝不进导出/日志/git**；**登录 cookie 绝不落盘**（红线不变）。
